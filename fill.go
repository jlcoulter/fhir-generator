...
func (g *Generator) fillChild(child *fhir.ElementDefinition, tree *fhir.ElementTree, out map[string]any, depth int) error {
	...
	// Decide whether to fill based on cardinality and fill mode.
	if !g.shouldFill(child) {
		return nil
	}
...
func (g *Generator) fillTypedValue(elem *fhir.ElementDefinition, typeCode string, tree *fhir.ElementTree, depth int) (any, error) {
	...
	if strings.HasPrefix(typeCode, "http://hl7.org/fhirpath/System.") {
		typeCode = strings.ToLower(strings.TrimPrefix(typeCode, "http://hl7.org/fhirpath/System."))
	}
	switch typeCode {
	case "":
		return nil, nil
